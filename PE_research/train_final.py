...
train_1 = folder1_name[split1_2:] 
valid_0 = folder_name[:split]
valid_1 = folder1_name[:split1]
test_0 = folder_name[split:split_2]
test_1 = folder1_name[split1:split1_2]





def get_data(folder0, folder1):
    class0_path = []
    class1_path = []
    target0 = []
    target1 = []
    for x in folder0:
        id = x.split('/')[-1]
        class0 = glob.glob('/home/mel/PE_research/PE100/non_PE/'+ id + '/0/*dcm')
        for i in class0:
...
train_data, _, _ = get_data(train_0, train_1)
valid_data,pe0, pe1 = get_data(valid_0, valid_1)
test_data, _, _ = get_data(test_0, test_1)
...
for epoch in range(num_epochs):
    ...
    if tp/pe1 > 0.7 and tn>best:
        ...
        save_best({'epoch':epoch,
                    ...
                    os.path.join(output_path,'savemodel'),'1214_Fbest_acc')
        best_acc = valid_acc
        best = tn


net, _, _ = load_model(net, optimizer, 'log/savemodel/1214_Fbest_acc_.pth')
