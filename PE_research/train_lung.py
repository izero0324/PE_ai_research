...
folder_name = list(glob.glob('/home/mel/PE_research/PE100/non_PE/NCKU/*')) + list(glob.glob('/home/mel/PE_research/PE100/non_PE/open/*'))
...
def get_data(folder0, folder1, repeat):
    ...
    for x in folder0:
        ...
        class1 = glob.glob(x + '/0/*dcm')
        #print(place)
        #class0 = class0.remove(class1)
        for i in class1:
            lung_list.append(i.split('/')[-1])
            class1_path.append(i)
            target1.append(torch.tensor([0.0,1.0]).long())

        for i in class0:
            if i.split('/')[-1] not in lung_list:
    ...
    for x in folder1:
        ...
        class1 = glob.glob(x + '/1/*dcm') + glob.glob(x + '/0/*dcm')
        for i in class1:
            lung_list.append(i.split('/')[-1])
            class1_path.append(i)
            target1.append(torch.tensor([0.0,1.0]).long())
...
train_data, _, _ = get_data(train_0, train_1, 2)
...
def test(model, data_loader, loss):
    ...
    return float(correct) / len(data_loader.dataset), correct_pe, correct_normal
...
for epoch in range(num_epochs):
    ...
    if tp/pe1 > 0.9 and tn>best:
        ...
        save_best({'epoch':epoch,
                    ...
                    os.path.join(output_path,'savemodel'),'0118_Lbest_acc')
        best_acc = valid_acc
        best = tn


net, _, _ = load_model(net, optimizer, 'log/savemodel/0118_Lbest_acc_.pth')
