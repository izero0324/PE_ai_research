...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride = 1, downsample = None,
                 dilation = (1,1), residual = True, BatchNorm = None, expansion = 2, drop_connect_rate = None):
...
def drn_d_base(BatchNorm, pretrained = False):
    model = DRN(Bottleneck_base, [1,1,3,4,6,3,1,1], arch = 'D', BatchNorm = BatchNorm)
        
    return model

def drn_d_base2(BatchNorm, pretrained = False):
    model = DRN(Bottleneck_base, [1,1,3,4,6,3,1,1], arch = 'D', BatchNorm = BatchNorm)
