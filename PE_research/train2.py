...
import torchvision
from model.drn import drn_d_54, drnSE_d_54
...
class Focal_Loss(_Loss):
    def __init__(self, alpha = 3, gamma = 5, logits = False, reduce = False, ignore_index=10000, from_logits=False):
        ...
        self.logits = logits
        self.reduce = reduce
        self.loss  = nn.BCELoss(weight = torch.Tensor([1.0, 4.0]).to(device))
...
class CNNX(nn.Module):
    ...
    def __init__(self, backbone = 'drn', out_stride = 16, num_class = 2):
        ...
        if backbone == 'drn':
            output_stride = 8
        self.drn = drnSE_d_54(nn.BatchNorm2d)
...
class remix_loss(nn.Module):
    ...
    def forward(self, inputs, targets):
        loss = self.loss1(inputs, targets) + self.loss2(inputs, targets)
...
for epoch in range(num_epochs):
    ...
    if valid_acc>best_acc:
        ...
        save_best({'epoch':epoch,
                    ...
                    os.path.join(output_path,'savemodel'),'1109_basebest_acc')
        best_acc = valid_acc


net, _, _ = load_model(net, optimizer, 'log/savemodel/1109_basebest_acc_.pth')
